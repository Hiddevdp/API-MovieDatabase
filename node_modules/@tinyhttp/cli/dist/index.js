...
import { installPackages, fileFetcher, setPackageJsonName, httpHeaders } from './utils.js';
...
cli
    ...
    process.chdir(dir);
    const res = await fetch(`https://api.github.com/repos/talentlessguy/tinyhttp/contents/examples/${name}`, httpHeaders);
    const data = (await res.json());
    if (res.status === 404)
        return console.error(`Template ${name} not found`);
    await fileFetcher(data, res.status);
    setPackageJsonName(dir);
    await installPackages(pkg);
    // Finish
    blue(`Done! You can now launch your project with running these commands:
